...
func TestHandleSemanticElementActionWidgets(t *testing.T) {
	...
	if !handleSemanticElementAction(edit, map[string]any{
		"action": "set_text",
		"text":   "alpha",
	}) {
	...
	if !handleSemanticElementAction(radio, map[string]any{
		"action": "select",
		"index":  1,
	}) {
	...
	if !handleSemanticElementAction(list, map[string]any{
		"action": "select",
		"index": 2,
	}) {
	...
	if !handleSemanticElementAction(combo, map[string]any{
		"action": "select",
		"index": 1,
	}) {
...
func TestHandleSemanticChildrenActionFindsTarget(t *testing.T) {
	...
	if !handleSemanticChildrenAction(dlg.GetChildren(), vtui.SemanticID(edit), map[string]any{
		"action": "set_text",
		"text":   "nested",
	}) {
	...
	if handleSemanticChildrenAction(dlg.GetChildren(), "missing-target", map[string]any{
		"action": "focus",
	}) {
...
func TestHandleSemanticFrameAction(t *testing.T) {
	...
	if !handleSemanticFrameAction(dlg, vtui.SemanticID(edit), map[string]any{
